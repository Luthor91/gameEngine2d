// gcc -I/usr/include/SDL2 -o animation_test Examples/SDL2/animation_test.c Core/Graphics/src/sprite.c Core/Graphics/src/tilemap.c Core/Graphics/src/aspect.c Core/Graphics/src/window.c Core/Graphics/src/frame.c Core/Graphics/src/animation.c -lSDL2 -lSDL2_image -lm && ./animation_test
...
Animation* Animation_Init(SDL_Renderer* renderer, Sprite* sprite, SDL_Rect* target, int tile_width, int tile_height, int max_frame, int speed) {
    ...
    for (int y = 0; y < num_rows; y++) {
        for (int x = 0; x < num_cols; x++) {
            ...
            Frame* frame = Frame_Init(&origin, target, frame_texture, 0.0f, frame_index);
