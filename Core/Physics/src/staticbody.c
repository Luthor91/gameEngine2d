...
StaticBodyManager* STATICBODY_MANAGER = NULL;
...
StaticBodyManager* StaticBodyManager_Init(int max_body) {
    ...
    manager->index = 0;
    manager->bodies = (StaticBody**)malloc(max_body * sizeof(StaticBody*));
...
void StaticBodyManager_Add(StaticBodyManager* manager, ...) {
    ...
    for (int i = 0; i < num_bodies; i++) {
        ...
        if (body == NULL) {
            fprintf(stderr, "StaticBodyManager_AddBodies: StaticBody at index %d is NULL\n", i);
            break;
        }

        if (manager->index >= manager->max_body) {
            fprintf(stderr, "StaticBodyManager_AddBodies: Maximum number of static bodies reached\n");
            break;
        }
...
void StaticBodyManager_Update(StaticBodyManager* manager, float deltaTime) {
    for (int i = 0; i < manager->max_body ; i++) {
        if (manager->bodies[i]) {
            StaticBody_Update(manager->bodies[i], deltaTime);
        }
    }
}
