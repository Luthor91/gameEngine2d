...
// Fonction de découpe de texture
Texture* Texture_Crop(Texture* original, SDL_Renderer* renderer, Point2D* pos, Size2D* size) {
    ...
    SDL_Texture* new_sdl_texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, rect.w, rect.h);
    if (!new_sdl_texture) {
        printf("Texture_Crop: Erreur de création de texture SDL: %s\n", SDL_GetError());
        return NULL;
    }

    SDL_SetRenderTarget(renderer, new_sdl_texture);
    SDL_RenderCopy(renderer, original->sdl_texture, &rect, NULL);
    ...
    new_texture->size->width = rect.w;
    new_texture->size->height = rect.h;
